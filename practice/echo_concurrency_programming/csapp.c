...
int open_clientfd(char *hostname, char *port)
{
    ...
    */
    ...
    hints.ai_flags = AI_NUMERICSERV; /* ... using a numeric port arg. */ // service인자를 포트 번호로 받는다.
    ...
    for (p = listp; p; p = p->ai_next)
    {
        ...
        if ((clientfd = socket(p->ai_family, p->ai_socktype, p->ai_protocol)) < 0) // 식별자를 리턴 못하고 -1 리턴한다면,
            continue;                                                              /* Socket failed, try the next */

        /* Connect to the server */
        if (connect(clientfd, p->ai_addr, p->ai_addrlen) != -1) // socket()으로 만든 클라이언트 호스트의 소켓의 식별자, 서버의 주소를 넣고 서버와 연결을 시도한다.
            break;                                              /* Success */
        if (close(clientfd) < 0)                                /* 연결 실패하면 소켓 닫아주고 다음 소켓 진행 */
...
int open_listenfd(char *port)
{
    ...
    memset(&hints, 0, sizeof(struct addrinfo));
    hints.ai_socktype = SOCK_STREAM; /* Accept connections */                // TCP 프로토콜
    hints.ai_flags = AI_PASSIVE | AI_ADDRCONFIG; /* ... on any IP address */ // 듣기 소켓으로 && 내가 사용하는 IP 형식에 맞는 IP 주소만
    hints.ai_flags |= AI_NUMERICSERV; /* ... using port number */            // service 인자를 port 번호만 받겠다
